...
Diferentemente de abordagens que agrupam agentes por comportamento comum (e.g., um agente planejador), Silva et al. adotam agentes orientados por tarefa (\textit{task-driven}), cada um composto por cinco m\'odulos cognitivos interconectados (Figura~\ref{fig:agent}). A \textbf{Mem\'oria H\'ibrida} atua como hub central, integrando: \textit{Working Memory} (curto prazo, contexto da itera\c{c}\~ao atual), \textit{Mem\'oria Sem\^antica} (longo prazo, baseada em RAG \cite{lewis2021retrieval} com OpenSearch) e \textit{Mem\'oria Procedural} (hist\'orico de intera\c{c}\~oes com o LLM). O m\'odulo de \textbf{Planejamento} encapsula a tomada de decis\~ao. A \textbf{Execu\c{c}\~ao} transforma planos em a\c{c}\~oes concretas via ferramentas externas. A \textbf{Percep\c{c}\~ao} enriquece o conhecimento do agente a partir do ambiente e da Mem\'oria Compartilhada. O \textbf{Racioc\'inio} gerencia as intera\c{c}\~oes com o LLM e o c\'odigo determin\'istico do agente.
