...
\begin{tikzpicture}[node distance=0.4cm and 0.4cm]
  ...
  % ── Orchestrator ──
  \node[orchestrator, right=0.7cm of user] (orch) {Agente\\Orquestrador};

  % ── Task-driven agents ──
  \node[agent, right=0.8cm of orch, yshift=0.5cm] (ag1) {Agente 1};
  \node[agent, right=0.8cm of orch] (ag2) {Agente 2};
  \node[agent, right=0.8cm of orch, yshift=-0.5cm] (agn) {Agente $N$};

  % ── Agents group fit ──
  \begin{scope}[on background layer]
    \node[draw, dashed, rounded corners=4pt, fill=blue!3,
          fit=(ag1)(agn), inner xsep=5pt, inner ysep=5pt,
          label={[font=\sffamily\tiny, anchor=south]above:Task-driven Agents}] (aggroup) {};
  \end{scope}

  % ── External tools ──
  \node[external, right=0.6cm of aggroup] (tools) {Ferramentas\\Externas};

  % ── Shared Memory ──
  \node[memory, below=0.4cm of aggroup, minimum width=2.8cm] (shmem) {Mem\'oria Compartilhada};
...
\begin{tikzpicture}[node distance=0.3cm and 0.4cm]
  ...
  \node[memory, minimum width=3.6cm, minimum height=1.5cm] (hybrid) {};
  \node[font=\sffamily\tiny\bfseries, anchor=north] at (hybrid.north) {Mem\'oria H\'ibrida};

  % Memory sub-components
  \node[memmodule, anchor=north west] at ($(hybrid.north west)+(0.15,-0.3)$) (wm) {Trabalho};
  \node[memmodule, right=0.1cm of wm] (sem) {Sem\^antica\\(RAG)};
  \node[memmodule, right=0.1cm of sem] (proc) {Procedural};
  ...
  % ── LLM and Agent Code ──
  \node[external, below left=0.25cm and 0.1cm of reason, minimum width=0.8cm] (llm) {LLM};
  \node[external, below right=0.25cm and 0.1cm of reason, minimum width=0.8cm] (code) {C\'odigo\\do Agente};

  % ── External tools box ──
  \node[external, right=0.4cm of exec] (exttools) {Ferramentas\\Externas};

  % ── Arrows ──
  \draw[dataflow] (percep) -- (hybrid);
  \draw[dataflow] (hybrid) -- (plan);
  \draw[dataflow] (plan.east) -- ++(0.15,0) |- (exec.north);
...
\begin{tikzpicture}[node distance=0.25cm and 0.25cm]
  ...
  \node[orchestrator, right=0.25cm of usr1] (ma1) {MA};
  \node[agent, right=0.25cm of ma1, fill=green!8] (af) {AF};
  \node[orchestrator, right=0.25cm of af] (ma2) {MA};
  \node[agent, right=0.25cm of ma2, fill=violet!8] (ac) {AC};
  \node[orchestrator, right=0.25cm of ac] (ma3) {MA};
  \node[agent, right=0.25cm of ma3] (usr2) {Usu\'ario};
  ...
  % ── External tool ──
  \node[external, above=0.3cm of af] (yf) {\textit{yfinance}};
  \draw[secondary] (af) -- (yf);

  % ── Shared Memory ──
  \node[memory, below=0.55cm of ma2, minimum width=5.5cm, minimum height=0.7cm] (shmem) {};
  \node[font=\sffamily\tiny\bfseries, anchor=south] at (shmem.south) {Mem\'oria Compartilhada};

  % Memory items
  \node[memoryitem] at ($(shmem.center)+(-1.8,0.03)$) (df) {DataFrames};
  \node[memoryitem] at ($(shmem.center)+(0,0.03)$) (fn) {Fun\c{c}\~ao Python};
  \node[memoryitem] at ($(shmem.center)+(1.7,0.03)$) (fig) {Gr\'afico Plotly};

  % Store arrows
  \draw[storearrow] (af.south) -- ++(0,-0.2) -| (df.north);
  \draw[storearrow] (ac.south) -- ++(0,-0.12) -| (fn.north);
  \draw[storearrow] (ac.south) -- ++(0,-0.12) -| (fig.north);
\end{tikzpicture}
