...
\author{Jean Reinhold\inst{1} \and Rafael Boeira\inst{1} \and Bernardo Ferr\~ao\inst{1} \and Pedro Rosa\inst{1}}
...
A arquitetura proposta por Silva et al. \cite{silva2025llm} organiza-se em tr\^es camadas (Figura~\ref{fig:overview}): (i)~uma camada de \textbf{intera\c{c}\~ao com o usu\'ario}; (ii)~um \textbf{Agente Orquestrador}, respons\'avel por distribuir tarefas e mediar a comunica\c{c}\~ao; e (iii)~\textbf{\textit{Task-driven Agents}} especializados que acessam ferramentas externas e compartilham uma \textbf{Mem\'oria Compartilhada} que funciona como \textit{blackboard}.
...
\begin{tikzpicture}[node distance=0.4cm and 0.5cm]
  ...
  \draw[secondary] ($(aggroup.south)+(-0.5,0)$) -- ($(shmem.north)+(-0.5,0)$);
...
\begin{tikzpicture}[node distance=0.35cm and 0.45cm]
  % ── Central: Hybrid Memory (wider to give sub-modules room) ──
  \node[memory, minimum width=5.2cm, minimum height=1.8cm] (hybrid) {};
  \node[font=\sffamily\scriptsize\bfseries, anchor=north, yshift=-2pt] at (hybrid.north) {Mem\'oria H\'ibrida};

  % Memory sub-components (spread wider, pushed into bottom half)
  \node[memmodule] at ($(hybrid.center)+(-1.7, -0.15)$) (wm) {Trabalho};
  \node[memmodule] at ($(hybrid.center)+(0, -0.15)$) (sem) {Sem\^antica\\(RAG)};
  \node[memmodule] at ($(hybrid.center)+(1.7, -0.15)$) (proc) {Procedural};

  % ── Surrounding modules ──
  \node[module, left=0.5cm of hybrid] (percep) {Percep\c{c}\~ao};
  \node[module, above=0.35cm of hybrid] (plan) {Planejamento};
  \node[module, right=0.5cm of hybrid] (exec) {Execu\c{c}\~ao};
  \node[module, below=0.35cm of hybrid, minimum width=1.5cm] (reason) {Racioc\'inio};

  % ── LLM and Agent Code ──
  \node[external, below left=0.25cm and 0.15cm of reason, minimum width=0.8cm] (llm) {LLM};
  \node[external, below right=0.25cm and 0.15cm of reason, minimum width=0.8cm] (code) {C\'odigo\\do Agente};

  % ── Agent boundary (excludes exttools) ──
  \begin{scope}[on background layer]
    \node[draw, densely dashed, rounded corners=5pt, fill=blue!2,
          fit=(percep)(plan)(exec)(reason)(hybrid)(llm)(code),
          inner sep=7pt,
          label={[font=\sffamily\tiny\bfseries, anchor=south east]north east:Task-driven Agent}] (agentbox) {};
  \end{scope}

  % ── External tools box (outside agent boundary) ──
  \node[external, right=0.35cm of agentbox.east |- exec] (exttools) {Ferramentas\\Externas};

  % ── Arrows ──
  \draw[dataflow] (percep) -- (hybrid);
  \draw[dataflow] (hybrid) -- (plan);
  \draw[dataflow] (plan.east) -- ++(0.25,0) |- (exec.north);
...
A viabilidade da arquitetura foi demonstrada em um cen\'ario financeiro (Python, LangGraph, GPT-4o) com tr\^es agentes (Figura~\ref{fig:report}): \textbf{Agente Moderador} (MA), que atua como orquestrador; \textbf{Agente Financeiro} (AF), que coleta dados via \textit{yfinance}; e \textbf{Agente de C\'odigo} (AC), que gera visualiza\c{c}\~oes com \textit{Plotly}. A coordena\c{c}\~ao segue o paradigma \textit{Report} \cite{becker2024multi}: cada agente reporta ao Moderador, que decide o pr\'oximo passo, enquanto a Mem\'oria Compartilhada armazena progressivamente os artefatos gerados.
...
\begin{tikzpicture}[node distance=0.3cm and 0.3cm]
  ...
  % ── Shared Memory (taller box: title on top, items well below) ──
  \node[memory, below=0.8cm of ma2, minimum width=6.2cm, minimum height=1.4cm] (shmem) {};
  \node[font=\sffamily\scriptsize\bfseries, anchor=north, yshift=-3pt] at (shmem.north) {Mem\'oria Compartilhada};

  % Memory items (pushed to bottom half of the box)
  \node[memoryitem] at ($(shmem.south)+(-2.0, 0.25)$) (df) {DataFrames};
  \node[memoryitem] at ($(shmem.south)+(0, 0.25)$) (fn) {Fun\c{c}\~ao Python};
  \node[memoryitem] at ($(shmem.south)+(2.0, 0.25)$) (fig) {Gr\'afico Plotly};

  % Store arrows (well-separated vertical drops)
  \draw[storearrow] (af.south) -- ++(0,-0.35) -| (df.north);
  \draw[storearrow] ($(ac.south)+(-0.08,0)$) -- ++(0,-0.2) -| (fn.north);
  \draw[storearrow] ($(ac.south)+(0.08,0)$) -- ++(0,-0.45) -| (fig.north);
\end{tikzpicture}
